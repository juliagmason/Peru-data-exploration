...
peru.catch <- read.csv("trips_main.csv")
peru.new <- read.csv("trips_sep8.csv") %>%  filter (!Trip.Code %in% peru.catch$Trip.code)
...
peru.sci.names <- read.csv ("Peru_species_sci_names.csv")
