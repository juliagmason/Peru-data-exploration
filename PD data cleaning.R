...
trips <- read.csv("trips_main_fixed.csv")
new.trips <- read.csv ("trips_sep8.csv") %>% filter (!Trip.Code %in% trips$Trip.code)
...
new.turt <- read.csv ("turtles_sep8.csv") %>% filter (!Trip.code %in% turtles$Trip.code)
...
sharks <- read.csv("sharks_main.csv") # messed with shark_main already
sharks$Lance.code <- paste(sharks$trip.code, sharks$lance, sep = ".")
#sharks <- left_join(sharks, set.DLL, by = "Lance.code")

mammals <- read.csv("Mammals_Main.csv")
new.mamm <- read.csv("mammals_sep8.csv") %>% filter (!Trip.code %in% mammals$Trip.code)
...
birds <- read.csv("seabirds_sep8.csv", stringsAsFactors=FALSE, fileEncoding="latin1") %>% filter (When.captured. != 3) # remove just sightings. Changed fileEncoding to encoding on windows
birds$Lance.code <- paste(birds$Trip.code, birds$Lance.., sep = ".")
birds <- left_join (birds, set.DLL, by = "Lance.code")

other <- read.csv("Other_species_sep8.csv") 
other$Lance.code <- paste(other$Trip.code, other$Lance.., sep = ".")
other <- left_join (other, set.DLL, by = "Lance.code")

mola <- read.csv("sunfish_sep8.csv") ## remember to filter for just sightings
mola$Lance.code <- paste(mola$trip, mola$set, sep = ".")
mola <- left_join (mola, set.DLL, by = "Lance.code")

seahorse <- read.csv("seahorses_sep8.csv")
